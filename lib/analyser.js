...
module.exports = {
  ...
  /**
   * Checks if component has been allowlisted and shouldn't be treated as unused.
   ...
   * ```
   *  allowlist: ['car-card-suv', 'car-card-sport', 'car-card-sedan']
   * ```
   ...
   * ```
   *
   * @param {string} componentOnAllowlist - name of component
   ...
   */
  isAllowlisted(componentOnAllowlist) {
    let hasWildcard = componentOnAllowlist.indexOf('*') !== -1;

    let unusedComponents = Object.values(this.components).filter(c => c.stats.count === 0);

    if (hasWildcard) {
      componentOnAllowlist = componentOnAllowlist.replace('*', '');

      unusedComponents.forEach(unusedComponent => {
        if (unusedComponent.name.indexOf(componentOnAllowlist) !== -1) {
          unusedComponent.allowlisted = true;
        }
      });
    } else {
      return Object.values(this.unusedComponents).find(
        component => component.name === componentOnAllowlist
      );
    }
  },
  ...
  unusedComponents() {
    let values = Object.values(this.components).filter(c => {
      return c.stats.count === 0 && !c.allowlisted;
    });
   ...
   */
  logResults(showStats, showOccurrences, allowlist) {
      ...
      Object.keys(this.components).forEach(key => {
        ...
        if (!component.isSubComponent && component.stats.count > 0) {
          ...
          if (allowlist.includes(key)) {
            console.log(colors.red(`    - allowlisted`));
          }
  ...
  /**
   * Marks components as allowlisted for unused components list
   *
   * @param {array} allowlist
   * @private
   */
  respectAllowlist(allowlist) {
    if (Array.isArray(allowlist)) {
      allowlist.forEach(componentOnAllowlist => {
        let hasWildcard = componentOnAllowlist.indexOf('*') !== -1;
        let unusedComponents = Object.values(this.components).filter(c => c.stats.count === 0);
        ...
        if (hasWildcard) {
          ...
          unusedComponents.forEach(unusedComponent => {
            if (unusedComponent.key.indexOf(componentOnAllowlist) !== -1) {
              unusedComponent.allowlisted = true;
            }
          });
        } else {
          let selectedComponents = Object.values(this.components).filter(
            component => component.key === componentOnAllowlist
          );

          selectedComponents.forEach(c => (c.allowlisted = true));
        }
      });
    }
  },
